...
本项目对应智能体课程大作业，属于“基于智能体技术开展应用开发”方向。系统不训练新的大语言模型，而是通过 API 调用已有大语言模型生成分析计划，并使用本地 Python 程序完成数据处理、统计计算、绘图和报告生成。
...
第二版采用简化设计：默认上传文件、用户输入和模型输出均可正常使用，不设计警告与错误提醒流程。
