...
汇总用户问题、数据画像、清洗记录、Planner 的分析计划、统计结果和图表，生成一份 Markdown 分析报告。该脚本只组织和格式化已有结果，不重新执行统计计算，也不再调用单独的结果解释 Skill。
...
其中“分析结论”直接根据已有统计结果生成简洁的模板化说明。例如根据 `p_value`、`alpha` 和 `significant` 字段组织结论，不另设 `result_interpreter.py`。
